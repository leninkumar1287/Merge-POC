...
router.post("/EUSD", cors(), async (req, res) => {
  ...
  try {
    ...
    let receipt = await EUSDContract.methods
...
router.post("/EINR", cors(), async (req, res) => {
  ...
  try {
    ...
    let receipt = await EINRContract.methods
      .mint(address, web3.utils.toWei("" + amount, "ether"))
      .send(transactionObject);
...
router.post("/EGOLD", cors(), async (req, res) => {
  ...
  try {
    ...
    if (from.includes("INR")) {
      ...
      receipt = await EGOLDContract.methods
        .buyEGoldINR(
          web3.utils.toWei("" + amountEGold, "ether"),
          address,
          StoH(orderDetails.id)
        )
        .send(transactionObject);
    } else {
      amountEGold =
